...
begin
  ...
  for r in
    ...
    join public.profiles u on (u.id = e.created_by or u.id = e.assigned_to or e.assigned_to is null)
  ...
  loop
    ...
    perform net.http_post(supabase_url||'/functions/v1/send-push', req_body, '{}'::jsonb, req_headers);
  end loop;

  for r in
    select distinct t.id as task_id, null::uuid as event_id, t.title, u.id as to_user_id
    from public.tasks t
    join public.profiles u on (u.id = t.created_by or u.id = t.assigned_to or t.assigned_to is null)
